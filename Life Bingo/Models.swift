...
struct BingoTask: Identifiable, Codable, Hashable {
    ...
    init(
        ...
        durationSec: Int = 45,
        observable: String = "",
        successProbability: Double = 0.8
    ) {
        self.taskId = taskId
        self.mapsToStep = mapsToStep
...
struct HabitGuideStep: Identifiable, Codable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Backward compatibility: previously stored as [String]
        if let newTasks = try container.decodeIfPresent([BingoTask].self, forKey: .bingoTasks) {
            bingoTasks = newTasks
        } else {
            let legacy = try container.decodeIfPresent([String].self, forKey: .bingoTasks) ?? []
            let sid = stepId
            bingoTasks = legacy.enumerated().map { idx, t in
                BingoTask(taskId: "\(sid)-T\(idx + 1)", mapsToStep: sid, text: t, durationSec: 45, observable: "完成：\(t)", successProbability: 0.75)
            }
        }
    }
